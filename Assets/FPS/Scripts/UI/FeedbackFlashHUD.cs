...
public class FeedbackFlashHUD : MonoBehaviour
{
    ...
    public float starsMaxAlpha = .8f;
    public float pulsatingStarsFrequency = 4f;
    ...
    private void Update()
    {
        ...
        if (player.starIsOn)
        {
            starsCanvasGroup.gameObject.SetActive(true);
            starsCanvasGroup.alpha = ((Mathf.Sin(Time.time * pulsatingStarsFrequency) / 2) + 0.5f) * starsMaxAlpha;
        }
        else
        {
            starsCanvasGroup.gameObject.SetActive(false);
        }
    }
